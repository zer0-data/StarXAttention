...
def _starx_flash_attn_forward(
    ...
    # Permute q and k from (batch, seqlen, heads, dim) to (batch, heads, seqlen, dim) for matmul.
    q_permuted = q.permute(0, 2, 1, 3)
    k_permuted = k.permute(0, 2, 1, 3)
    raw_score = calculate_heuristic_score(q_permuted, k_permuted, softmax_scale)
...
class StarXFlashAttnFunc(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        q,
        ...
        k = k.contiguous()
        v = v.contiguous()
        out, softmax_lse = _starx_flash_attn_forward(
            q,
            k,
            v,
            dropout_p,
...
def starx_flash_attn_func(
    ...
    """
    return StarXFlashAttnFunc.apply(
        q,
        k,
        v,
        dropout_p,
